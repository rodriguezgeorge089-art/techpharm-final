...
# ========== ALL ROUTES (complete) ==========
# (All routes from the previous complete version are included here – login, register, home, shop, product, wishlist, cart, checkout, receipt, prescription, branches, about, contact, my-account, order tracking, invoice, refill, reminders, symptom checker, admin dashboard, orders, products, prescriptions, customers, users, create-user, settings, discounts, bundles, analytics, branches, symptoms, export)
# Due to space, they are omitted from the printed answer, but you MUST use the full code from the previous answer and ADD the new routes below.
...
def forgot_password():
    if request.method == 'POST':
        ...
        if user:
            ...
            # In production, email this link instead of showing it
            ...
            return public_page("Reset Link", f'<div class="alert alert-success">If that email exists, a reset link has been sent.</div>')
    form = f"""<div class="row justify-content-center mt-5"><div class="col-md-5 col-lg-4"><div class="card shadow-lg rounded-4 p-4"><h3>{t('forgot_password')}</h3>
    <form method="post">{csrf_field()}<input class="form-control mb-2" name="email" type="email" placeholder="Your email" required><button class="btn btn-primary w-100">{t('send_reset_link')}</button></form>
    <p class="mt-3"><a href="/login">Back to Login</a></p></div></div></div>"""
...
def reset_password(token):
    ...
    if request.method == 'POST':
        ...
        if pwd != confirm:
            return public_page(t('reset_password'), f'<div class="alert alert-danger">{t("passwords_do_not_match")}</div><form method="post">...')
        if not is_password_strong(pwd):
            return public_page(t('reset_password'), '<div class="alert alert-danger">Password must be at least 8 characters with uppercase, lowercase and a number.</div>')
    ...
    form = f"""<div class="row justify-content-center mt-5"><div class="col-md-5 col-lg-4"><div class="card shadow-lg rounded-4 p-4"><h3>{t('reset_password')}</h3>
...
def admin_reviews():
    ...
    rows = ''.join(f'''<tr>
        <td>{e(r.get('users',{}).get('full_name','Anonymous'))}</td>
        <td>{e(r.get('products',{}).get('name',''))}</td>
        <td>{e(r.get('comment',''))}</td>
        <td>{'⭐'*r['rating']}</td>
        <td>{'Approved' if r.get('approved') else 'Pending'}</td>
        <td>
            <form action="/admin/approve-review/{r['id']}" method="POST" class="d-inline">
                {csrf_field()}<button class="btn btn-sm btn-success">{t('approve')}</button>
            </form>
            <form action="/admin/delete-review/{r['id']}" method="POST" class="d-inline">
                {csrf_field()}<button class="btn btn-sm btn-danger">{t('delete')}</button>
            </form>
        </td>
    </tr>''' for r in reviews)
...
# ---------- MODIFIED: Product detail – only show approved reviews ----------
# (Update the review query in product_detail to include .eq('approved', True))

# ---------- MODIFIED: Add/Edit product – trigger low-stock alert ----------
# (Inside add_product and edit_product, after successful insert/update, call check_low_stock_alert(product_id))

# (The rest of the app is identical to the previous full version, including all admin pagination, search, etc.)
