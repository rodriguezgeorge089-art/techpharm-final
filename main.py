...
def prescription_upload():
    if request.method=='POST':
        ...
        return public_page("Prescription Received",'<div class="text-center mt-5"><i class="fas fa-check-circle fa-5x text-success mb-3"></i><h2>Thank You!</h2><p>Your prescription has been submitted.</p><a href="/" class="btn btn-primary rounded-pill mt-3">Back to Home</a></div>')
...
def admin_orders():
    ...
    for o in orders:
        ...
        <div class="d-flex align-items-center"><form method="post" action="/admin/order/{o["id"]}/status" class="d-flex"><select name="status" class="form-select form-select-sm me-2" style="width:auto;"><option {sel_pending}>pending</option><option {sel_confirmed}>confirmed</option><option {sel_shipped}>shipped</option><option {sel_delivered}>delivered</option></select><button class="btn btn-sm btn-primary">Update</button></form>
        <a href="/admin/order/{o['id']}/invoice" target="_blank" class="btn btn-sm btn-outline-primary ms-2">Invoice</a></div></td></tr>'''
...
def admin_invoice(oid):
    ...
    return f"""<!DOCTYPE html><html><head><title>Invoice #{oid}</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>
...
<div class="invoice-box">
    <div class="no-print mb-3 text-end"><button onclick="window.print()" class="btn btn-primary rounded-pill"><i class="fas fa-print me-2"></i> Print Invoice</button></div>
