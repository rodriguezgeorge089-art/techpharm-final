...
<a href="/supplier/orders" class="btn btn-info mb-4"><i class="fas fa-list"></i> Purchase Orders</a>
<div class="row">{cards}</div></div></body></html>"""
...
# ---------- Supplier Catalog Browse for Sellers ----------
...
def supplier_add_form(request: Request):
    ...
    if not profile or profile['role'] != 'supplier': return RedirectResponse("/login")
    nav = navbar(profile)
    return HTMLResponse(SUPPLIER_ADD_PAGE.replace("{navigation}", nav))
...
def supplier_edit_form(request: Request, product_id: str):
    ...
    nav = navbar(profile)
    return HTMLResponse(SUPPLIER_EDIT_PAGE.replace("{navigation}", nav).format(**prod.data))
...
def purchase_checkout(request: Request):
    ...
    for supplier_id, items in by_supplier.items():
        ...
        for it in items:
            ...
            prod = service_supabase.table("supplier_products").select("stock_available").eq("id", it["product_id"]).single().execute()
            if prod.data:
                new_stock = prod.data["stock_available"] - it["quantity"]
                service_supabase.table("supplier_products").update({"stock_available": new_stock}).eq("id", it["product_id"]).execute()
