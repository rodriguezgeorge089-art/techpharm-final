...
LOGIN_HTML = f"""<!DOCTYPE html><html><head><title>Login</title>{BOOTSTRAP}</head><body>
{NAVBAR_GUEST}
...
SIGNUP_HTML = f"""<!DOCTYPE html><html><head><title>Sign Up</title>{BOOTSTRAP}</head><body>
{NAVBAR_GUEST}
<div class="container" style="max-width:400px;">
...
DASHBOARD_HTML = f"""<!DOCTYPE html><html><head><title>Dashboard</title>{BOOTSTRAP}</head><body>
{NAVBAR_LOGGED_IN}
<div class="container">
  <h2>Welcome, {{full_name}} (<span class="badge bg-info">{{role}}</span>)</h2>
  <div class="row mt-4">
    <div class="col-md-4"><a href="/products" class="btn btn-outline-primary w-100">Browse Products</a></div>
    <div class="col-md-4"><a href="/cart" class="btn btn-outline-success w-100">View Cart</a></div>
    <div class="col-md-4"><a href="/orders" class="btn btn-outline-warning w-100">My Orders</a></div>
  </div>
</div></body></html>"""

PRODUCTS_HTML = f"""<!DOCTYPE html><html><head><title>Products</title>{BOOTSTRAP}</head><body>
{NAVBAR_LOGGED_IN}
<div class="container">
  <h2>OTC Products</h2>
...
<div class="col-md-4 mb-3">
  <div class="card h-100">
    <div class="card-body">
      ...
      <a href="/cart/add/{id}" class="btn btn-primary">Add to Cart</a>
    </div>
  </div>
</div>"""

SELLER_HTML = f"""<!DOCTYPE html><html><head><title>My Shop</title>{BOOTSTRAP}</head><body>
{NAVBAR_LOGGED_IN}
<div class="container">
  <h2>My Products</h2>
...
ADD_PRODUCT_HTML = f"""<!DOCTYPE html><html><head><title>Add Product</title>{BOOTSTRAP}</head><body>
{NAVBAR_LOGGED_IN}
...
EDIT_PRODUCT_HTML = f"""<!DOCTYPE html><html><head><title>Edit Product</title>{BOOTSTRAP}</head><body>
{NAVBAR_LOGGED_IN}
<div class="container" style="max-width:500px;">
  <h2>Edit Product</h2>
  <form method="post">
...
CART_HTML = f"""<!DOCTYPE html><html><head><title>Cart</title>{BOOTSTRAP}</head><body>
{NAVBAR_LOGGED_IN}
<div class="container">
  <h2>Your Cart</h2>
  {{cart_items}}
...
ORDERS_HTML = f"""<!DOCTYPE html><html><head><title>Orders</title>{BOOTSTRAP}</head><body>
{NAVBAR_LOGGED_IN}
<div class="container">
  <h2>My Orders</h2>
  {{order_list}}
...
def do_login(request: Request, email: str = Form(...), password: str = Form(...)):
    try:
        ...
        return HTMLResponse(LOGIN_HTML.replace("</body>", "<div class='alert alert-danger m-3'>Login failed</div></body>"))
...
def do_signup(full_name: str = Form(...), email: str = Form(...), password: str = Form(...), role: str = Form(...)):
    try:
        ...
        return HTMLResponse(SIGNUP_HTML.replace("</body>", f"<div class='alert alert-danger m-3'>Error: {e}</div></body>"))
...
def products_page(request: Request, search: str = ""):
    ...
    return HTMLResponse(PRODUCTS_HTML.replace("{product_cards}", cards))
...
def seller_dashboard(request: Request):
    ...
    if profile.data['role'] != 'seller':
        return HTMLResponse("<div class='alert alert-danger'>Only sellers can access this page.</div>")
    products = sup.table("products").select("*").eq("seller_id", profile.data['id']).execute()
    cards = "".join([SELLER_PRODUCT_CARD.format(**p) for p in (products.data or [])])
    return HTMLResponse(SELLER_HTML.replace("{product_cards}", cards))
...
def update_product(request: Request, product_id: str, name: str = Form(...), description: str = Form(""),
                   category: str = Form(...), price: float = Form(...), stock: int = Form(...)):
    sup = get_valid_session(request)
    if not sup: return RedirectResponse("/login")
...
def view_cart(request: Request):
    ...
    return HTMLResponse(CART_HTML.replace("{cart_items}", items_html).replace("{total}", str(round(total, 2))))
...
def checkout(request: Request, payment_method: str = Form("cash_on_delivery")):
    ...
    try:
        ...
        for item in cart:
            ...
            total += subtotal
            order_items_data.append({"product_id": item["product_id"], "quantity": item["quantity"], "unit_price": price})
            sup.table("products").update({"stock": product.data["stock"] - item["quantity"]}).eq("id", item["product_id"]).execute()
        ...
        return HTMLResponse(f"<div class='alert alert-danger'>Checkout error: {e}</div><a href='/cart'>Back</a>")
...
def order_history(request: Request):
    ...
    for order in orders.data:
        ...
        orders_html += ORDER_ITEM_HTML.format(
            order_id=order["id"][:8], status=order["status"], total=order["total_amount"],
            payment_method=order.get("payment_method", "N/A"), date=order["created_at"][:10],
